...
namespace SDN
{
	...
	public:
		...
		void process(juce::AudioBuffer<float>& sourceBuffer, int numInputChannels);
	...
	private:
		...
		void processSample(juce::AudioBuffer<float>& sourceBuffer, const float* currentReadPointer, int sampleIndex, int maxIndex);
